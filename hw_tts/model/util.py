...
class ScaledDotProductAttention(nn.Module):
    ...
    def forward(self, q, k, v, mask=None):
        ...
        qk = torch.bmm(q, k.transpose(-2, -1)) / self.temperature
        if mask is not None:
            qk.masked_fill_(mask.view(qk.shape), -torch.inf)

        attn = self.dropout(torch.softmax(qk / self.temperature, -1))
...
class PositionwiseFeedForward(nn.Module):
    ...
    def __init__(self, d_in, d_hid, model_config, dropout=0.1):
        ...
        kernels, padding = model_config.fft_conv1d_kernel, model_config.fft_conv1d_padding

        # Use Conv1D
        # position-wise
        self.w_1 = nn.Conv1d(
            d_in, d_hid, kernel_size=kernels[0], padding=padding[0])
        # position-wise
        self.w_2 = nn.Conv1d(
            d_hid, d_in, kernel_size=kernels[1], padding=padding[1])
